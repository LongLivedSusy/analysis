...
def mkcanvaslimit(name='c1'):
	c1 = TCanvas(name,name,800,630)
        c1.SetBottomMargin(.13)
...
def stamp(lumi='35.6', showlumi = True):
        tl.SetTextFont(cmsTextFont)
        tl.SetTextSize(0.98*tl.GetTextSize())
        tl.DrawLatex(0.138,0.925,'CMS') # 'CMS') for thesis                                                                                     
        tl.SetTextFont(extraTextFont)
        tl.SetTextSize(1.0/0.98*tl.GetTextSize())
        xlab = 0.213
#       tl.DrawLatex(xlab,0.915, ('MC' in datamc)*' simulation '+'preliminary')                                                              
        tl.SetTextFont(regularfont)
        tl.SetTextSize(.85*tl.GetTextSize())
        thingy = ''
        if showlumi: thingy+=str(lumi)+' fb^{-1}(#sqrt{s}=13 TeV)'
        xthing = 0.6
        if not showlumi: xthing+=0.13
        tl.DrawLatex(xthing,0.931,thingy)
	tl.SetTextSize(.85/1*tl.GetTextSize())

	tl.SetTextSize(.92*tl.GetTextSize())
	process = 'pp #rightarrow #tilde{g}#tilde{g}, c#tau_{0}(#chi^{#pm}_{1})= 50 cm'
	BR1 = 'BR(#tilde{g} #rightarrow q#bar{q} #chi^{#pm}_{1})=67%'
	BR2 = 'BR(#tilde{g} #rightarrow q#bar{q} #chi^{0}_{1})=33%'
	tl.DrawLatex(0.152,0.86,process)
	tl.DrawLatex(0.152,0.8,BR1)
	tl.DrawLatex(0.152,0.75,BR2)
	tl.SetTextSize(.2/1*tl.GetTextSize())
