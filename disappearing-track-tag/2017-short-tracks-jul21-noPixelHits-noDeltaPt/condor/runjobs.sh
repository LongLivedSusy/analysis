#!/bin/bash
    ...
    cd /nfs/dust/cms/user/kutznerv/shorttrack/analysis/disappearing-track-tag
    export PYTHONDONTWRITEBYTECODE=1
    export PYTHONPATH=$PYTHONPATH:$(pwd)/../
    export PYTHONPATH=$PYTHONPATH:$(pwd)/../tools
    export PYTHONPATH=$PYTHONPATH:$(pwd)/../../tools
    PROCESSNUM=$(($1 + 1))
    CMD=$(sed ''"$PROCESSNUM"'q;d' 2017-short-tracks-jul21-noPixelHits-noDeltaPt/condor/args)
    echo $CMD
    eval $CMD
    if [ $? -eq 0 ]
    then
