...
int tmva(const char* signalFileName, const char* outFileName, const char* backgroundPath, const char* maxTreeEntries) {
    ...
    if (strcmp(maxTreeEntriesString.c_str(), "-1")) {
        ...
        mycuts=("pt>15 && abs(eta)<2.4 && passPFCandVeto==1 && trkRelIso<0.2 && dzVtx<0.1 && ptErrOverPt2<10 && nMissingMiddleHits==0 && nMissingOuterHits>=2 && trackQualityHighPurity==1 && Entry$<" + maxTreeEntriesString).c_str();
        mycutb=("pt>15 && abs(eta)<2.4 && passPFCandVeto==1 && trkRelIso<0.2 && dzVtx<0.1 && ptErrOverPt2<10 && nMissingMiddleHits==0 && nMissingOuterHits>=2 && trackQualityHighPurity==1 && Entry$<" + maxTreeEntriesString).c_str();
    } else {
        mycuts=("pt>15 && abs(eta)<2.4 && passPFCandVeto==1 && trkRelIso<0.2 && dzVtx<0.1 && ptErrOverPt2<10 && nMissingMiddleHits==0 && nMissingOuterHits>=2 && trackQualityHighPurity==1");
        mycutb=("pt>15 && abs(eta)<2.4 && passPFCandVeto==1 && trkRelIso<0.2 && dzVtx<0.1 && ptErrOverPt2<10 && nMissingMiddleHits==0 && nMissingOuterHits>=2 && trackQualityHighPurity==1");
    }
