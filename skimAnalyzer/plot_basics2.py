...
binnings["tracks_ptErrOverPt2"] = [20, 0, 0.1, "p_{T} / p_{T}^{2} (1/GeV)"]
...
def plot2D(variable, cutstring, histos, lumi, pdffile, ymin=1e-1, ymax=1e5, showdata = False, drawoption = "colz"):
    ...
    for label in histos:
        ...
        if "CombinedBg" in label or "Signal" in label:
            ...
            shared_utils.stamp()
            canvas.SaveAs(pdffile.split("/")[-1] + "_" + label.replace(":", "_").replace(" ", "_") + ".pdf")
            ...
            canvas.SaveAs(pdffile.split("/")[-1] + "_" + label.replace(":", "_").replace(" ", "_") + "_lines.pdf")


def plot1D(variable, cutstring, histos, lumi, pdffile, xlabel = False, ymin=1e-1, ymax=1e5, showdata = False):
    ...
    if contains_data and showdata:
        ...
        empty_histo = TH1D("empty", "empty", example.GetXaxis().GetNbins(), example.GetXaxis().GetBinLowEdge(1), example.GetXaxis().GetBinLowEdge(example.GetXaxis().GetNbins()+1))
        shared_utils.histoStyler(empty_histo)
        
        hratio, pad1, pad2 = shared_utils.FabDraw(canvas, legend, empty_histo, histolist, lumi = lumi, datamc = 'MC')
