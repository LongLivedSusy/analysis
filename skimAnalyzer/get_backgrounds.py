...
for dedx in ["", "_SidebandDeDx", "_MidDeDx", "_HighDeDx"]:
    ...
    for category in ["short", "long"]:
        ...
        histos["_sr%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_SR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), ""]
        #histos["_srgenfake%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_SR_%s==1 && tracks_fake==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), ""]
        #histos["_srgenprompt%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_SR_%s==1 && tracks_fake==0 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), ""]
        histos["_fakecr%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), ""]
        #histos["_fakeprediction%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), "fakerate_HT_n_allvertices_FakeRateDet_fakerate_%s" % category]

        # added iso cut on FR CR:
        #histos["_fakecrIso%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_trkRelIso<0.01 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), ""]
        ...
        ## other FRs:
        #histos["_fakeprediction_HT%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), "fakerate_HT_FakeRateDet_fakerate_%s" % category]
        #histos["_fakeprediction_n_allvertices%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), "fakerate_n_allvertices_FakeRateDet_fakerate_%s" % category]
        #histos["_fakeprediction_n_goodjets%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), "fakerate_n_goodjets_FakeRateDet_fakerate_%s" % category]
        #histos["_fakeprediction_MinDeltaPhiMhtJets%s_%s" % (dedx, category)] = [" && tracks_is_pixel_track==%s && tracks_CR_%s==1 && tracks_deDxHarmonic2pixel>%s && tracks_deDxHarmonic2pixel<%s" % (is_pixel_track, category, lower, upper), "fakerate_MinDeltaPhiMhtJets_FakeRateDet_fakerate_%s" % category]
...
if __name__ == "__main__":
    ...
    #variables = ['n_goodjets', 'n_allvertices', 'tracks_pt', 'tracks_eta', 'tracks_phi', 'n_btags', 'HT', 'HT:n_allvertices']
    variables = ['HT:n_allvertices']
    regions = ["FakeRateDet"]
        ...
        for variable in variables:
            for h_suffix in histos:
                ...
                if "Baseline" in region:
                    ...
                    parameters.append([variable, event_selections[region] + cuts, scaling, region + "_Run2016GHJetHT", h_suffix, folder, Run2016GH, output_root_file])
