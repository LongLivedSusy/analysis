...
if isdata: fout = fout.replace('.root','_cute.root')
...
for key in sorted(keys):#[:241]:
	...
	#hPromptShortSElValidationZLL_MatchedCaloTruth

	if not 'SElValidationZLL' in name: continue
	...
	if not isdata: 
		...
		htarget_promptcontam = infile.Get(name.replace('CaloSideband_','_').replace('hPrompt','hFake'))
	...
	if not isdata: 
		...
		htarget.Add(infile.Get(name.replace('CaloAideband_','_').replace('hPrompt','hFake')))
	...
	if isdata: pdfname = pdfname.replace('.','_cute.')
	#c1.Print(pdfname)
	c1.Delete()
