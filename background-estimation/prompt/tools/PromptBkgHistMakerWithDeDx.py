...
for ientry in range(nentries):
	...
	for ipi, pi in enumerate(c.TAPPionTracks):
		...
		if not c.TAPPionTracks_mT[ipi]<85: continue #this is kind of the one thing different about the control than the T&P		
	...
	if singleElEvent_:
		elec, elcharge, eldedxPixel, eldedxStrips = random.sample(SmearedElectrons,1)[0]
	...
	if singleMuEvent_:
		muon, mucharge, mudedxPixel, mudedxStrips = random.sample(SmearedMuons,1)[0]
		adjustedMht = TLorentzVector()
		adjustedMht.SetPxPyPzE(0,0,0,0)
		adjustedJets = []
		adjustedHt = 0
		...
		fv = [adjustedHt,adjustedMht.Pt(),adjustedNJets,adjustedBTags, 1+len(disappearingTracks),1+nShort,nLong, mindphi, log10dedxmass,len(SmearedElectrons), len(SmearedMuons)-1, len(SmearedPions),mudedxPixel,pt,eta]
		fv.append(getBinNumber(fv))
	...
	if singlePiEvent_:
		pion, picharge, pidedxPixel, pidedxStrips = random.sample(SmearedPions,1)[0]		
		adjustedMht = TLorentzVector()
		adjustedMht.SetPxPyPzE(0,0,0,0)
		adjustedJets = []
		adjustedHt = 0
