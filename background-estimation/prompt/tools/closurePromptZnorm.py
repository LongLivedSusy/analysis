...
fout = 'closureAbcdZnorm_phase'+str(phase)+'.root'
if mycutedatatest: fout = fout.replace('.root','_cute.root')

fnew = TFile(fout,'recreate')


hnamenormstem = 'hElSElValidationZLLZone0p0To2p5_NJets'
hnormstem_num = infile.Get(hnamenormstem+'Truth')
hnormstem_den = infile.Get(hnamenormstem+'Control')

normfactor = hnormstem_num.Integral()/hnormstem_den.Integral()
print 'got normfactor', normfactor
...
for key in sorted(keys):#[:241]:
	...
	zone = name.split('Zone')[-1].split('_')[0] # for this Z version, use the zone only to fill in the prediction histogram
