...
def main():
	...
	varfilename = "LLSUSY_cuts/%s.cuts" %RGS_var
	if not os.path.exists(varfilename):
		error("unable to open variables file %s" % varfilename)

	# Name of signal file
	#sigfilename = "<path to your signal file>"
	sigfilename = "/u/user/sangilpark/WorkDir/DisappearingTracks/CMSDAS2018/longlivedLE/CMSSW_10_1_0/src/cmsdas2018/tools/skim/skim_pMSSM12_MCMC1_%s_step4_TREEMAKER_RA2AnalysisTree.root" %sys.argv[1]
	if not os.path.exists(sigfilename):
		error("unable to open signal file %s" % sigfilename)

	# Name of background file        
	bkgfilename = "/u/user/sangilpark/WorkDir/DisappearingTracks/CMSDAS2018/InputsForDas/Skims/Background/skim_allBackgrounds.root"
	...
	selection  = "(Mht >= 150 && NTags==1 && MinDeltaPhiMhtJets > 0.2)"
	...
	if not os.path.exists("./LLSUSY_RGSOutput"):
	    os.system("mkdir -p LLSUSY_RGSOutput")

	rgsfilename = "LLSUSY_RGSOutput/%s.root" % (NAME+"_"+RGS_var)
