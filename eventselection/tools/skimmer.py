...
def main(event_tree_filenames, track_tree_output, nevents = -1, treename = "TreeMaker2/PreSelection", verbose = False, iEv_start = False, only_tagged_tracks = True, save_cleaned_variables = False):
    ...
    if phase == 0:
        mask_file = TFile("../../tools/usefulthings/Masks.root", "open")
        if is_data:
    ...
    histograms = {}
    if is_signal:
        histograms["neutralino_gluino_mass"] = TH2D("neutralino_gluino_mass", "neutralino_gluino_mass", 4000, 0, 4000, 4000, 0, 4000)
    ...
    for iEv, event in enumerate(tree):
        ...
        if is_signal:
            ...
            for mass in list(set(masses)):
                if len(masses) == 2:
                    ...
                    tree_branch_values["signal_lsp_mass"][0] = int(masses["neutralino"])
        ...
        for i, electron in enumerate(event.Electrons):
            if electron.Pt() > 30 and abs(electron.Eta()) < 2.4 and bool(event.Electrons_mediumID[i]):
                ...
                lepton_level_output.append({"leptons_pt": electron.Pt(),
                                             ...
                                             "leptons_id": 11,
                                             "leptons_mtw": event.Electrons_MTW[i],
                                             })
        ...
        for iCand, track in enumerate(event.tracks):
            ...
            if track.Pt() < 30 or not shared_utils.isBaselineTrack(track, iCand, event, h_mask) or not pass_pion_veto(event, iCand, deltaR=0.03):
                continue
            ...
            # if track is tagged and there is are leptons in the event
            #   a) get the invariant mass
            #   b) check if m_inv(DT, lepton) is within the Z mass peak
            invariant_masses = []
            leptons_dr = []
            for lepton in goodleptons["electrons"] + goodleptons["muons"]:
                    invariant_mass = (track + lepton).M()
                    invariant_masses.append(invariant_mass)
                    deltaR = track.DeltaR(lepton)
                    leptons_dr.append(deltaR)

            for i in range(len(invariant_masses)):
                if (i+1)<6:
                    #print "invariant_mass for track", iCand, ":", invariant_masses[i]
                    track_level_output[-1]["tracks_invmass%s" % (i+1)] = invariant_masses[i]
                    #print "leptons_dr", leptons_dr[i]
                    track_level_output[-1]["tracks_leptondr%s" % (i+1)] = leptons_dr[i]

            # check if invariant mass is inside in the Z mass peak:
            for invariant_mass in invariant_masses:
                if abs(invariant_mass) > (91-10) and abs(invariant_mass) < (91+10):
                    track_level_output[-1]["tracks_zmassveto"] = 1
